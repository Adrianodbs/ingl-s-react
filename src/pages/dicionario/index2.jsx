...
function DicionarioPage() {
  ...
  const [palavra, setPalavra] =useState('nada')
  const [traducao, setTraducao] =useState('nadinha')
  const [uso, setUso] =useState('teste')
          ...
          <tbody className="tabela-dicionario">
            {palavrasTreinamento.map((p)=>(
              <tr className='tr__pai' key={p.Palavra}>
                  <td>{p.Palavra}</td>
                  <td>{p.Traducao}</td>
                  <button onClick={togglePostModal}>Ver</button>
